...
#✨✨✨
from LSTM_5 import generate_model
...
def train(args, epoch, model, train_loader, device, optimizer, criterion):
    ...
    #✨✨✨
    path = './pth_file/LSTM5_' + str(epoch+5) + '_1.pth'
    torch.save(model, path)
    path = './pth_file/LSTM5_' + str(epoch+5) + '_2.pth'
...
def val(args, model, val_loader, device, criterion, best_acc):
    ...
    torch.save(model.state_dict(), '/pth_file%s.pth'%val_acc)
...
if __name__ == '__main__':
    ...
    #✨✨✨
    torch.save(model, 'LSTM5_1.pth')
    torch.save(model.state_dict(), 'LSTM5_2.pth')
